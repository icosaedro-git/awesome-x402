...
- [Usenami](https://agentic.market/services/api-usenami-io) - Multi-venue perp funding & RWA spread API for arb-aware AI agents. 4 pay-per-call endpoints at $0.001 USDC each on Base mainnet: per-venue funding rates across 30+ exchanges (`/v1/funding/current`), cross-venue funding spread for a ticker (`/v1/perp/funding-spread?ticker=BTC`), oracle price-source family classification for basis-risk awareness (`/v1/perp/oracle-families`), and RWA perpetual coverage on Hyperliquid HIP-3 DEXes — stocks, metals, forex, commodities, pre-IPO synthetics (`/v1/rwa/perp-coverage`). Differentiated vs CoinGecko/CMC by perp-first focus + RWA inclusion. x402 v2 with Bazaar discovery extension via Coinbase CDP Facilitator. MCP server with 4 paid + 2 free tools at [namixai/usenami-mcp](https://github.com/namixai/usenami-mcp). ([API base](https://api.usenami.io)) ([Provider](https://usenami.io))
